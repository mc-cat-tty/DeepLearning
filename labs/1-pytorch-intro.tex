...
\texttt{torch.Tensor} is the central class of the package. Setting \texttt{.requires\_grad} as True, PyTorch starts to keeping track of
...
To stop a tensor from tracking operations, we can call \texttt{.detach()}; alternatively we can run code inside \texttt{torch.no\_grad()}
context.