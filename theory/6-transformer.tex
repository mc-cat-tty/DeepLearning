...
Remember that sequential models always work with numbers (scalars, vectors, matrices, and tensors), for e.g., words are converted into embeddings.
...
at time $t$, the discrete (causal) 1D convolution is defined by:
$\omega * f(x) = \sum_{i=0}^{k-1} \omega(i) f(x-i)$.

\obsbox{
This convolution is moving backward in time, not forward.
}
...
\begin{itemize}
 ...
 \item RNN cannot be parallelized, while CNN and TCN can be (convolution can be inherently parallelized) making them faster.
...
Clearly, we can get rid of the for loop and the summation working with $K$ and $V$ in matrix form.
...
&\rightarrow \quad Q, K, V \in \mathbb{R}^{d_{\text{tokens}} \times d_{k}}
\end{align*}
