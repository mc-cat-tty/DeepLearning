...
$$\mathcal{L}(\theta|D) = \sum_{i=1}^N [y_i \log P(y=1|x) + (1-y_i) \log P(y=0|x)]$$
...
The cost function used in gradient descent is called objective or loss function, and it is typically denoted with $L(\cdot)$ or $J(\cdot)$.

This is the vanilla rule: \textbf{batch gradient descent}.
Is called batch since we have to run the average $\frac{1}{N} \sum_{i=1}^N [\dots]$ through all $N$ training examples.
...
To avoid being stuck to local minimums and traveling zig-zags with SGD, we use strategies like \textbf{ADAM} -- Adaptive Moment Estimation.
