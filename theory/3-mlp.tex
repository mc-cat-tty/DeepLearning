...
If we add a non-linearity to the function $f(x_i, W, b) = \sigma (W x_i + b)$ (\textbf{Logistic Regression}) we obtain the results of the previous
section. We can still train the "model" using a binary cross-entropy loss:
$$
L(W, b) = - \frac{1}{m} \sum_{i=1}^m [y_i \log(\sigma (W x_i + b)) + (1 - y_i) \log(1 - \sigma (W x_i + b))]
$$
...
$$
L(\theta; D) = \frac{1}{N} \sum_{i=0}^N L_i(\theta; D) + \lambda \Omega(\theta)
$$
...
\begin{itemize}
 ...
 \begin{itemize}
   \item \textbf{Hinge loss}: $\hat f = f(x_i) \in \mathbb{R}^K, \ L_i = \sum_{j \neq y_i} \max(0, \hat f_j - \hat f_{y_i} + 1)$.
   ...
   \item \textbf{Softmax loss}: $\hat f = f(x_i) \in \mathbb{R}^K, \  L_i = - \log (\frac{e^{\hat  f_{y_i}}}{\sum_j e^{\hat f_j}})$.
 ...
 \begin{itemize}
   \item \textbf{Mean Squared Error (MSE)}: $L_i = \| \hat f - y_i \|_2^2$.
...
$$
\theta^* = \arg \min_\theta \left[ L(\theta; D) \right] = \arg \min_\theta \left[ \frac{1}{N} \sum_{i=0}^{N-1} L_i(\theta; D) + \lambda \Omega(\theta) \right]
$$
